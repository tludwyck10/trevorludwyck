...
// Single combined portfolio gallery — interleaved so the page reads as one
// body of portrait work rather than two categorized sets.
...
// portfolio gallery so returning from home doesn't repeat images.
export const home = Array.from({ length: 7 }, (_, i) => {
  const [w, h] = ratioFor(i + 1);
  return { src: placeholder(`tlp-home-${i + 1}`, w, h), alt: `Placeholder — portrait ${i + 1}`, href: '/portfolio/gallery/' };
});

// Portfolio cover — a studio portrait (black and white, free Unsplash
